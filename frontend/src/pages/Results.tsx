...
const getRelativeTime = (submittedAt: string | null | undefined, startedAt: string | null | undefined): string => {
  if (!submittedAt || !startedAt) return "Not submitted";
  const start = new Date(startedAt).getTime();
  const submit = new Date(submittedAt).getTime();
  const diffMinutes = Math.floor((submit - start) / 60000);
  if (diffMinutes < 0) return "Not submitted";
  return `${diffMinutes}:${String(Math.floor(((submit - start) % 60000) / 1000)).padStart(2, "0")} min from start`;
};
...
export default function Results() {
  ...
  const { players, questions, submissions, winner, startedAt } = battle;
                      ...
                      <div className="flex items-center gap-3 mt-1 flex-wrap">
                        ...
                        {isPlayer1 ? (
                          <span className="text-xs text-muted-foreground">
                            {question.difficulty === "EASY" && getRelativeTime(myPlayer?.player1EasyLastSubmissionTime as string, startedAt as string)}
                            {question.difficulty === "MEDIUM" && getRelativeTime(myPlayer?.player1MediumLastSubmissionTime as string, startedAt as string)}
                            {question.difficulty === "HARD" && getRelativeTime(myPlayer?.player1HardLastSubmissionTime as string, startedAt as string)}
                          </span>
                        ) : (
                          <span className="text-xs text-muted-foreground">
                            {question.difficulty === "EASY" && getRelativeTime(myPlayer?.player2EasyLastSubmissionTime as string, startedAt as string)}
                            {question.difficulty === "MEDIUM" && getRelativeTime(myPlayer?.player2MediumLastSubmissionTime as string, startedAt as string)}
                            {question.difficulty === "HARD" && getRelativeTime(myPlayer?.player2HardLastSubmissionTime as string, startedAt as string)}
                          </span>
                        )}
                      </div>
                    </div>
